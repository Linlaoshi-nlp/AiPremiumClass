...
if __name__ == '__main__':
    ...
    #构建词汇表
    vocab, w2idx = build_from_doc(comments_data)
    print('词汇表大小:', len(w2idx))
    ...
    def convert_data(batch_data):
        ...
        for comment,vote in batch_data:
            comments.append(torch.tensor([w2idx.get(word, w2idx['UNK'])for word in comment]))
            votes.append(vote)
    ...
    print(len(vocab) == len(w2idx))  # 必须输出 True

        #保存模型
    torch.save(model.state_dict(),'dmsc_comments_classifier.pth')

        #保存词典
    torch.save(w2idx,'dmsc_vocab.pth')
